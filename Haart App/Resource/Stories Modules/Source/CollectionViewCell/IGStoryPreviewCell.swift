...
final class IGStoryPreviewCell: UICollectionViewCell, UIScrollViewDelegate {
            ...
            case .forward:
                if snapIndex < story?.snapsCount ?? 0 {
                    if let snap = story?.snaps[snapIndex] {
                        ...
                        let timeInterval = Double(snap.lastUpdated)
                        // create NSDate from Double (NSTimeInterval)
                        let myNSDate = Date(timeIntervalSince1970: timeInterval)
                        //let sec = myNSDate.getAgoTimeForStories().2
                        let min = myNSDate.getAgoTimeForStories().1
                        let hour = myNSDate.getAgoTimeForStories().0
                        if(hour >= 1) {
                            
                            storyHeaderView.lastUpdatedLabel.text = "\(hour.string)h"
                        }
                        else if(min >= 1) {
                            storyHeaderView.lastUpdatedLabel.text = "\(min.string)m"
                        }
                        else {
                            storyHeaderView.lastUpdatedLabel.text = "Now"
                        }
                    }
                }
            case .backward:
    ...
    public var story: IGStory? {
        didSet {
            ...
            if let picture = story?.user.picture {
                storyHeaderView.snaperImageView.sd_imageIndicator = SDWebImageActivityIndicator.whiteLarge
                storyHeaderView.snaperImageView.sd_setImage(with: URL(string:picture), placeholderImage: nil)
    ...
    private func createSnapView() -> UIImageView {
        ...
        snapView.backgroundColor = .darkGray
...
//        storyHeaderView.snaperImageView.sd_imageIndicator = SDWebImageActivityIndicator.whiteLarge
//        storyHeaderView.snaperImageView.sd_setImage(with: URL.init(string: url)) { (img, error, abc, url) in
//            
//        }
//        
...
extension IGStoryPreviewCell: StoryPreviewHeaderProtocol {
    func didTapDeleteButton() {
        if let userStories = AppSettings.currentUserSnapshot!.data()["stories"] as? [String:Any] {
            SVProgressHUD.show()
            var snaps = userStories["snaps"] as? Array<[String:Any]> ?? Array<[String:Any]>()
            snaps.removeAll(where: { (snap) -> Bool in
                let snapId = snap["id"] as! String
                let currentSnapId = story!.snaps[snapIndex].internalIdentifier
                print(snapId)
                print(currentSnapId)
                let shouldDelete = currentSnapId  == snapId
                return shouldDelete
            })
            ...
            AppSettings.currentUserSnapshot!.reference.updateData(["stories":userStories], completion: { (error) in
                print("updated")
