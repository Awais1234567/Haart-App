...
public struct CPWheelLayoutConfiguration {
    ...
    public var yOffSet:CGFloat = 0 // only for top center type
    ...
    public init(withCellSize cellSize:CGSize,
                ...
                contentHeigthPadding:Double = 0.0, yOffSet:CGFloat = 0.0) {
...
open class CPCollectionViewWheelLayout: UICollectionViewLayout {
    ...
    required public init?(coder aDecoder: NSCoder) {
        self.configuration = CPWheelLayoutConfiguration.init(withCellSize: CGSize(width: 50, height: 50), radius: 100, angular: 30, wheelType: .leftBottom, yOffSet: 0.0)
    ...
    override open func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        var angle = angular/90.0*Double(visibleCellIndex)*(Double.pi / 2.0)
        ...
        case .leftBottom:
            ...
            if (angle <= ((Double.pi / 2.0)+2.0*angleOffset+angular/90.0)) && (angle >= -angleOffset) {
        ...
        case .topCenter:
            ...
            if (angle <= (Double.pi+2.0*angleOffset+angular/180.0)) && (angle >= -angleOffset) {
                attributes.isHidden = false
                translation = CGAffineTransform.init(translationX: -CGFloat(cos(angle)*radius),
                                                     y: configuration.yOffSet + (CGFloat(sin(angle)*radius)))
            }
        case .bottomCenter:
            attributes.center = CGPoint.init(x: (viewSize.width)/2.0,
                                             y: (contentOffset.y)+(viewSize.height)-cellSize.height/2)
        ...
        case .bottomCenter,.topCenter,.rightCenter,.leftCenter:
            fadeFactor = 1-fabs(angle-(Double.pi / 2.0))*0.5
        default:
            fadeFactor = 1-fabs(angle-(Double.pi / 4.0))
        }
        attributes.alpha = CGFloat(fadeAway ? (fadeFactor) : 1.0)
        attributes.size = CGSize(width:cellSize.width*CGFloat(fadeFactor), height:cellSize.height*CGFloat(fadeFactor)) /*CGSize(width:50, height:50)*/
        return attributes
    }
