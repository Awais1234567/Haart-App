...
class CallViewController: UIViewController, UICollectionViewDelegateFlowLayout {
    ...
    //Containers
    private var users = [QuickBloxUser]()
    ...
    override func viewDidLoad() {
        ...
        guard profile.isFull == true, let currentConferenceUser = QuickBloxProfile.currentUser() else {
            return
        }
    ...
    func setupViews(){
        view.addSubview(opponentsCollectionView)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let updateConnectionStatus: ((_ status: NetworkConnectionStatus) -> Void)? = { [weak self] status in
            ...
            if notConnection == true {
                self?.cancelCallAlertWith("Please check your Internet connection")
            }
        }
        QuickbloxReachability.instance.networkStatusBlock = { status in
...
extension CallViewController: QBRTCClientDelegate {
    ...
    func session(_ session: QBRTCBaseSession, updatedStatsReport report: QBRTCStatsReport, forUserID userID: NSNumber) {
        ...
        var existingUser: QuickBloxUser?
...
class CallingViewController: UIViewController{
    ...
    var opponentVideoView: UIView = {
        let view = UIView(frame: CGRect(x: 20, y: 100, width: 75, height: 150))
    ...
    private var statsUserID: UInt?
    private var disconnectedUsers = [UInt]()
    var currentUserVideoView: UIView!
    ...
    override func viewDidLoad() {
        ...
        currentUserVideoView = LocalVideoView(previewlayer: previewLayer)
        currentUserVideoView.frame = CGRect(x: 0, y: 50, width: 414, height: 736)
        currentUserVideoView.backgroundColor = UIColor.white
        currentUserVideoView.layer.cornerRadius = 10
        view.addSubview(currentUserVideoView)
        setupViews()
    }
    func setupViews(){
        view.addSubview(toolbar)
    ...
    private func userView(userID: UInt) -> UIView? {
        ...
        if profile.isFull == true, profile.ID == userID,
            ...
            if let result = videoViews[userID] as? QBRTCRemoteVideoView {
                ...
                //Opponents
                let remoteVideoView = QBRTCRemoteVideoView(frame: CGRect(x: 0, y: 50, width: 414, height: 736))
                remoteVideoView.videoGravity = AVLayerVideoGravity.resizeAspect.rawValue
                videoViews[userID] = remoteVideoView
                remoteVideoView.setVideoTrack(remoteVideoTraсk)
                self.view.addSubview(remoteVideoView)
                self.currentUserVideoView.superview?.bringSubviewToFront(currentUserVideoView)
                self.toolbar.superview?.bringSubviewToFront(toolbar)
                scaleCurrentUserView(scalableView: currentUserVideoView)
                return remoteVideoView
            }
        }
        return nil
    }
    func scaleCurrentUserView(scalableView: UIView){
        //let oldFrame = currentUserVideoView.frame
        UIView.animate(withDuration: 0.5, animations: {
            scalableView.frame = CGRect(x: 20, y: 100, width: 100, height: 200)
        })
    }
}

extension CallingViewController: LocalVideoViewDelegate {
    // MARK: LocalVideoViewDelegate
    func localVideoView(_ localVideoView: LocalVideoView, pressedSwitchButton sender: UIButton?) {
        let newPosition: AVCaptureDevice.Position = cameraCapture.position == .back ? .front : .back
        guard cameraCapture.hasCamera(for: newPosition) == true else {
...
extension CallingViewController: QBRTCAudioSessionDelegate {
    //MARK: QBRTCAudioSessionDelegate
    func audioSession(_ audioSession: QBRTCAudioSession, didChangeCurrentAudioDevice updatedAudioDevice: QBRTCAudioDevice) {
        let isSpeaker = updatedAudioDevice == .speaker
        dynamicButton.pressed = isSpeaker
...
// MARK: QBRTCClientDelegate
extension CallingViewController: QBRTCClientDelegate {
...
//            }
            ...
            if let user = createConferenceUser(userID: userID.uintValue) {
                ...
                if user.connectionState == .connected {
                    self.users.insert(user, at: 0)
                    var _ = userView(userID: userID.uintValue)
                    reloadContent()
                }
            } else {
                usersDataSource?.loadUser(userID.uintValue, completion: { [weak self] (user) in
...
extension CallingViewController: CallKitManagerDelegate{
    func callKitManager(_ callKitManager: CallKitManager, didUpdateSession session: QBRTCSession) {
        if self.session == nil {
            QBRTCClient.instance().add(self as QBRTCClientDelegate)
            //            QBRTCAudioSession.instance().addDelegate(self)
