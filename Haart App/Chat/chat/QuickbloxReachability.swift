...
class QuickbloxReachability: NSObject {
    // MARK: shared Instance
    static let instance: QuickbloxReachability = {
        let instance = QuickbloxReachability()
        instance.commonInit()
        return instance
    }()
    ...
    private func startReachabliyty() {
        ...
        context.info = UnsafeMutableRawPointer(Unmanaged<QuickbloxReachability>.passUnretained(self).toOpaque())
        ...
        let callbackClosure: SCNetworkReachabilityCallBack? = {
            ...
            let handler = Unmanaged<QuickbloxReachability>.fromOpaque(info).takeUnretainedValue()
