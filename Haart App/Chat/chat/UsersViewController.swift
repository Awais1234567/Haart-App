...
class UsersViewController: UITableViewController {
    ...
    //MARK: - IBOutlets
    lazy var audioCallButton: UIButton = {
        let button = UIButton(frame: CGRect(x: 0, y: 500, width: 100, height: 50))
        //button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle("Audio Call", for: .normal)
        button.setTitleColor(UIColor.red, for: .normal)
        button.addTarget(self, action: #selector(didPressAudioCall(_:)), for: .touchUpInside)
        return button
    }()
    lazy var videoCallButton: UIButton = {
        let button = UIButton(frame: CGRect(x: 314, y: 500, width: 100, height: 50))
        //button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle("Video Call", for: .normal)
        button.addTarget(self, action: #selector(didPressVideoCall(_:)), for: .touchUpInside)
        button.setTitleColor(UIColor.red, for: .normal)
        return button
    }()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        view.addSubview(audioCallButton)
    ...
    private func configureTableViewController() {
        tableView.register(UserTableViewCell.self, forCellReuseIdentifier: UsersDataSourceConstant.userCellIdentifier)
